...
class MovieUpcomingScreen extends ConsumerStatefulWidget {
  const MovieUpcomingScreen({Key? key}) : super(key: key);

  @override
  ConsumerState<MovieUpcomingScreen> createState() =>
      _MovieUpcomingScreenState();
}

class _MovieUpcomingScreenState extends ConsumerState<MovieUpcomingScreen>
  ...
  void initState() {
    ...
    _controller.addListener(() async {
      if (_controller.position.pixels >
        ...
        ref.read(movieUpcomingProvider.notifier).loadMorePost();
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final movieDiscover = ref.watch(movieUpcomingSearchProvider.notifier).state;
    return Consumer(builder: (ctx, watch, child) {
      ...
      return RefreshIndicator(
          ...
          child: MoviesWidget(
              text: "UPCOMING", request: "upcoming", movie: movieDiscover));
    });
  }
}
